/*
	...
	Created for: "Software Dev Bootcamp - DB Class #3 - Homework Assignment" Miles Technologies
	Date completed: 3/9/2019
...
**Added tables to track room sales (Problem #1)**
INSERTS test data into all tables
Displays all tables and
Answers to problem #2, #3, #4, #5, #6, #7, #8, #9
*/

--DROP DATABASE IF EXISTS TavernDatabase3;
--CREATE DATABASE TavernDatabase3;           --Creates the database used to contain tables (uncomment if needed)
--GO
...
Displays all tables
Answer to problem #2, #3, #4, #5, #6, #7, #8, #9

	**NOTE MULTIPLE ATTEMPTS ON PROBLEM #9**
*/

SELECT * FROM TavernSales;
SELECT * FROM InnRoomStatus;
...
SELECT guestBirthDay FROM TavernGuests
WHERE guestBirthDay < '2000-01-01' ORDER BY guestBirthDay ASC;
GO
...
SELECT DISTINCT guestName FROM TavernGuests;
GO
...
SELECT guestName FROM TavernGuests ORDER BY guestName ASC;
GO
...
SELECT TOP 10 * FROM TavernSales ORDER BY priceOfService DESC; 
GO

/*
7. Write a query to return all Lookup Table Names - this is, not the names of the tables but the 
Names of things like Status/Role/Class,etc. (w/ Union)
*/

SELECT locationName FROM TavernLocation
UNION
SELECT tavernName FROM TavernInfo
UNION
SELECT roleName FROM EmployeeRole
UNION
SELECT employeeName FROM EmployeeInfo
UNION
SELECT note FROM GuestNotes
UNION
SELECT guestStatusDescription FROM guestStatus
UNION
SELECT className FROM ClassTypes
UNION
SELECT guestName FROM TavernGuests
UNION
SELECT supplyName FROM SupplyStorage
UNION
SELECT serviceStatus FROM TavernServiceStatus
UNION
SELECT serviceName FROM TypesOfSevices
UNION
SELECT roomStatus FROM InnRoomStatus;
GO
...
SELECT CONCAT('Class: ', classId, ' ', 'Level: ', guestLevel)  --Guest Level 1-10
AS 'Level 1-10' FROM GuestClassLevel 
WHERE guestLevel BETWEEN 1 AND 10
ORDER BY guestLevel ASC
GO

SELECT CONCAT('Class: ', classId, ' ', 'Level: ', guestLevel)  --Guest Level 11-20
AS 'Level 11-20' FROM GuestClassLevel 
WHERE guestLevel BETWEEN 11 AND 20
ORDER BY guestLevel ASC
GO

SELECT CONCAT('Class: ', classId, ' ', 'Level: ', guestLevel)  --Guest Level 21-30
AS 'Level 21-30' FROM GuestClassLevel 
WHERE guestLevel BETWEEN 21 AND 30
ORDER BY guestLevel ASC
GO
